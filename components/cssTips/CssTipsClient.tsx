...
export default function CssTipsClient({ tips, initialTipId, initialTipData }: CssTipsClientProps) {
  const [selectedTip, setSelectedTip] = useState<CssTip | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  useEffect(() => {
    const fromData = initialTipData ?? null;
    if (fromData) {
      setSelectedTip(fromData);
      setIsModalOpen(true);
      return;
    }
    if (typeof initialTipId === "number") {
      const found = tips.find((t) => t.id === initialTipId) || null;
      if (found) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 mt-16 md:my-24 mb-8 md:mb-12">
